...
class DpoTpoDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if fmt == "dpo":
            ...
            if self.sdxl:
                out['caption'] = token_or_text
            else:
                out['input_ids'] = token_or_text
            return out
        else:
            img_path = rec.get('image') or rec.get('file_name') or rec.get('preferred')
            if img_path is None:
                raise ValueError(f"TPO record must have 'image', 'file_name', or 'preferred'. Keys: {list(rec.keys())}")
            im = self.transform(self._load_pil(img_path))
            pixel_values = torch.cat([im, im], dim=0)
            ...
            if self.sdxl:
                out['caption_unsafe'] = input_ids_unsafe
                out['caption_safe'] = input_ids_safe
            else:
                out['input_ids_unsafe'] = input_ids_unsafe
                out['input_ids_safe'] = input_ids_safe
            return out
